...
class AlgorithmCacheService {
  static final AlgorithmCacheService _instance = AlgorithmCacheService._internal();
  ...
  Future<void> cacheFollowingFeed({
    ...
    try {
      ...
      debugPrint('✅ Cached following feed for user $userId (${videos.length} videos)');
    } catch (e) {
  ...
  Future<CachedFeedResult?> getCachedFollowingFeed(String userId) async {
    try {
      ...
      final videos = (data['videos'] as List)
          .map((v) => _videoFromMap(v))
          .toList();
      final nextCursor = data['nextCursor'] as Map<String, dynamic>?;
      
      debugPrint('✅ Retrieved cached following feed for user $userId (${videos.length} videos)');
  ...
  Future<void> cacheForYouFeed({
    ...
    try {
      ...
      debugPrint('✅ Cached For You feed for user $userId (${videos.length} videos)');
    } catch (e) {
  ...
  Future<CachedFeedResult?> getCachedForYouFeed(String userId) async {
    try {
      ...
      debugPrint('✅ Retrieved cached For You feed for user $userId (${videos.length} videos)');
      return CachedFeedResult(videos: videos, nextCursor: nextCursor);
  ...
  /// Clean old cache entries
  Future<void> _cleanOldCacheEntries(SharedPreferences prefs, String keyPrefix) async {
    try {
      final keys = prefs.getKeys();
  ...
  Map<String, dynamic> _videoToMap(HomeVideo video) {
    return {
      ...
      'createdAt': DateTime.now().millisecondsSinceEpoch, // HomeVideo doesn't have createdAt field
    };
  }
