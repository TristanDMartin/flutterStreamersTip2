...
class DeviceCapabilityService {
  ...
  Future<int> getDeviceHeapSizeMB() async {
    ...
    try {
      if (Platform.isAndroid) {
        ...
        secureLog(
            '📱 DeviceCapability: Estimated Android heap: 512MB (balanced default)');
      } else if (Platform.isIOS) {
        // iOS devices generally have more memory available
        // Conservative default for iOS
        _cachedHeapSizeMB = 512;
        secureLog(
            '📱 DeviceCapability: Estimated iOS heap: 512MB (conservative)');
      } else {
  ...
  Future<String> getRecommendedResolution() async {
    ...
    if (heapSize <= 256) {
      ...
      secureLog(
          '📱 DeviceCapability: Recommended resolution: 720p (low memory device)');
    } else {
      _cachedRecommendedResolution = '1080';
      secureLog(
          '📱 DeviceCapability: Recommended resolution: 1080p (preferred sharp playback)');
    }
