...
class VideoCacheManager extends ChangeNotifier {
  ...
  Future<VideoPlayerController?> getPlayerItem(String videoURL) async {
    ...
    if (_assetCache.containsKey(videoURL)) {
      // appLog('📦 Using cached asset for: $videoURL');
  ...
  Future<VideoPlayerController?> createPlayerController(String videoURL) async {
    ...
    try {
      ...
      if (videoURL.startsWith('assets/')) {
        ...
        // appLog('❌ Invalid video URL: $videoURL');
        return null;
      }
      ...
      // appLog('✅ Created and cached controller for: $videoURL');
      return controller;
    } catch (error) {
      // appLog('❌ Error creating player controller for $videoURL: $error');
  ...
  Future<VideoPlayerController?> tryFallbackURLs(String originalURL) async {
    ...
    for (final fallbackURL in fallbackURLs) {
      try {
        ...
        // appLog('✅ Used fallback URL for $originalURL: $fallbackURL');
        return controller;
      } catch (error) {
        // appLog('⚠️ Fallback URL failed: $fallbackURL');
        continue;
      }
    }

    // appLog('❌ All fallback URLs failed for: $originalURL');
    return null;
  }

  Future<void> cacheController(
      VideoPlayerController controller, String videoURL) async {
    // Add to cache
    ...
    // appLog('📦 Cached controller for: $videoURL (Total: ${_assetCache.length})');
    notifyListeners();
  }
  ...
  void evictOldestItems() {
    ...
    if (_assetCache.isNotEmpty) {
      ...
      controller?.dispose();
      // appLog('🗑️ Evicted cached controller: $firstKey');
    }
  }
  ...
  void clearCacheOnMemoryWarning() {
    ...
    _assetCache.clear();
    // appLog('🧹 Cleared video cache due to memory warning');
    notifyListeners();
  }
  ...
  void clearCache() {
    ...
    _assetCache.clear();
    // appLog('🧹 Manually cleared video cache');
    notifyListeners();
  }
