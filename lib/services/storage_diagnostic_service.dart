    ...
    try {
      ...
      try {
        ...
        if (e.toString().contains('object-not-found')) {
          results['storage_access'] = 'success'; // This is expected for a test file
          debugPrint('✅ Storage access test passed (object-not-found is expected)');
        } else {
      ...
      try {
        ...
        if (e.toString().contains('storage/object-not-found')) {
          ...
          debugPrint('⚠️ Storage rules are restrictive (expected for test file)');
        } else {
  ...
  static String getRecommendations(Map<String, dynamic> results) {
    ...
    if (results['storage_access'] == 'failed') {
      ...
      if (error.contains('bucket-not-found')) {
        recommendations.add('• Firebase Storage bucket not found. Please set up Storage in Firebase Console.');
      } else if (error.contains('project-not-found')) {
        recommendations.add('• Firebase project not found. Please check your Firebase configuration.');
      } else if (error.contains('unauthorized')) {
        recommendations.add('• Storage access unauthorized. Please check your Firebase Storage rules.');
    ...
    if (results['storage_write'] == 'failed') {
      ...
      if (error.contains('unauthorized')) {
        recommendations.add('• Storage write unauthorized. Please check your Firebase Storage rules.');
      } else if (error.contains('quota-exceeded')) {
        recommendations.add('• Storage quota exceeded. Please check your Firebase project limits.');
      } else {
    ...
    if (results['storage_rules'] == 'error') {
      recommendations.add('• Storage rules configuration error. Please check your storage.rules file.');
    }
    
    if (recommendations.isEmpty) {
      recommendations.add('• All diagnostics passed! Firebase Storage should be working correctly.');
    }
