...
class OfflineInboxService {
  ...
  Future<void> cacheUserProfiles(
      Map<String, app_user.User> userProfiles) async {
    try {
      final prefs = await SharedPreferences.getInstance();
  ...
  Future<Map<String, app_user.User>> getCachedUserProfiles() async {
    try {
      ...
      return usersJson.map((userId, userData) => MapEntry(
          userId,
          app_user.User(
            id: userData['id'],
  ...
  Future<Map<String, int>> getCachedUnreadCounts() async {
    try {
      ...
      final Map<String, dynamic> unreadCountsJson =
          jsonDecode(unreadCountsJsonString);
  ...
  Future<Map<String, bool>> getCachedOnlineStatus() async {
    try {
      ...
      final Map<String, dynamic> onlineStatusJson =
          jsonDecode(onlineStatusJsonString);
