...
class OfflineService {
  ...
  Future<void> cacheNotifications(List<ActivityNotification> notifications) async {
    try {
  ...
  Future<List<ActivityNotification>> getCachedNotifications() async {
    try {
      ...
      return notificationsJson.map((json) => ActivityNotification.fromJson(json)).toList();
    } catch (e) {
    ...
    try {
      ...
      return {
        'notifications_count': notificationsString != null ? 
          (jsonDecode(notificationsString) as List).length : 0,
        'users_count': usersString != null ? 
          (jsonDecode(usersString) as List).length : 0,
        'last_sync': lastSync > 0 ? 
          DateTime.fromMillisecondsSinceEpoch(lastSync).toIso8601String() : 'Never',
