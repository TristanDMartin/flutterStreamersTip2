...
class SSLTestService {
  ...
  static Future<bool> testSSLConnection(
      {String url = 'https://www.google.com'}) async {
    try {
      ...
      debugPrint(
          '✅ SSLTestService: SSL connection successful (Status: ${response.statusCode})');
      client.close();
      return true;
    } catch (e) {
  ...
  static Future<bool> testFirebaseSSL() async {
    try {
      ...
      final client = HttpClient();
      final request =
          await client.getUrl(Uri.parse('https://firebase.googleapis.com'));
      final response = await request.close();

      debugPrint(
          '✅ SSLTestService: Firebase SSL connection successful (Status: ${response.statusCode})');
      client.close();
      return true;
    } catch (e) {
  ...
  static Future<Map<String, bool>> runAllTests() async {
    ...
    // Test Google APIs
    results['google_apis'] =
        await testSSLConnection(url: 'https://www.googleapis.com');
