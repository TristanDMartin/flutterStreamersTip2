...
class FirestoreScheduledPostService {
  ...
  Future<String> saveScheduledPost({
    ...
    try {
      ...
      final scheduledPostId = 'scheduled_${DateTime.now().millisecondsSinceEpoch}';
      ...
      final scheduledPostData = {
        ...
        'caption': caption,
        'tags': hashtags,
        'visibility': _privacyToPostVisibility(privacy).name, // Convert enum to string
        'videoId': videoId,
        'videoUrl': videoUrl,
  ...
  Future<List<ScheduledPost>> getScheduledPosts({
    ...
    try {
      final currentUser = _auth.currentUser;
      if (currentUser == null) {
        developer.log('⚠️ No authenticated user', name: 'FirestoreScheduledPostService');
        return [];
      }
      ...
      try {
        ...
        if (limit != null) {
          query = query.limit(limit * 2); // Get more to account for client-side filtering
        }
      ...
      developer.log('📋 Loaded ${snapshot.docs.length} scheduled posts from Firestore',
          name: 'FirestoreScheduledPostService');
  ...
  Future<void> updateScheduledPostStatus(
    ...
    try {
      ...
      developer.log('✅ Updated scheduled post status: $scheduledPostId -> $status',
          name: 'FirestoreScheduledPostService');
    } catch (e) {
  ...
  Future<void> deleteScheduledPost(String scheduledPostId) async {
    try {
      await _firestore.collection('scheduled_posts').doc(scheduledPostId).delete();
  ...
  Future<ScheduledPost> publishNow(String scheduledPostId) async {
    try {
      ...
      final updatedDoc = await _firestore.collection('scheduled_posts').doc(scheduledPostId).get();
  ...
  Future<List<Map<String, dynamic>>> getPostsReadyToPublish() async {
    try {
      ...
      developer.log('🔍 Checking for posts ready to publish (now: ${now.toDate()})',
          name: 'FirestoreScheduledPostService');
      ...
      try {
        snapshot = await query.get();
        developer.log('✅ Found ${snapshot.docs.length} posts ready to publish (using nested query)',
            name: 'FirestoreScheduledPostService');
      } catch (e) {
        // If nested field query fails (missing index), fall back to client-side filtering
        developer.log('⚠️ Nested field query failed (may need composite index), using fallback: $e',
            name: 'FirestoreScheduledPostService');
        
        // Get all scheduled posts and filter client-side
        ...
        developer.log('✅ Found ${readyPosts.length} posts ready to publish (using client-side filtering)',
            name: 'FirestoreScheduledPostService');
  ...
  ScheduledPost _mapToScheduledPost(String id, Map<String, dynamic> data) {
    ...
    if (data['schedule'] != null) {
      ...
      schedule = PostSchedule(
        ...
        perPlatform: (scheduleData['perPlatform'] as Map<String, dynamic>?)
                ?.map(
              (key, value) {
        ...
        createdAtUtc:
            (scheduleData['createdAtUtc'] as Timestamp).toDate(),
        updatedAtUtc:
            (scheduleData['updatedAtUtc'] as Timestamp).toDate(),
      );
    }
